...
struct BMP
{
    ...
    void fill_region(uint32_t x0, uint32_t y0, uint32_t w, uint32_t h, uint8_t B, uint8_t G, uint8_t R, uint8_t A)
    {
        ...
        {
        	//char buffer [200];
  			printf("Region out of range error X: %d Y: %d vs mX: %d. mY: %d", x0,y0,(uint32_t)bmp_info_header.width, (uint32_t)bmp_info_header.height);
    ...
    void set_pixel(uint32_t x, uint32_t y, uint8_t B, uint8_t G, uint8_t R, uint8_t A)
    {
        ...
        {
        	char buffer [200];
  			sprintf (buffer, "Pixel out of range error X: %d Y: %d vs mX: %d. mY: %d", x,y,(uint32_t)bmp_info_header.width, (uint32_t)bmp_info_header.height);
            throw std::runtime_error(buffer);
        }
